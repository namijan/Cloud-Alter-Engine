...
async function run() {
  try {
    ...
    const res = await axios.get(`https://developer.api.autodesk.com/modelderivative/v2/designdata/${urn}/manifest`, { headers: { Authorization: `Bearer ${token}`, 'x-ads-region': region } });
    console.log("Success with APAC region:", res.data.status);
  } catch(e) {
    console.error("Error with APAC region:", e.response?.status, e.response?.data || e.message);
  }
}
run();
